...
function resolveColor(value: any): ColorResolvable {
    ...
    if (cssColorNames[lower]) {
        return cssColorNames[lower] as ColorResolvable
    }

    if (isHexColor(value)) {
        return value.startsWith('#') ? value as ColorResolvable : `#${value}`
    }
